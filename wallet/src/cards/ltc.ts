...
//@ts-ignore
import * as bch from 'litecore-lib';
...
export default class LTC extends Card {
  ...
  async listUnspent(address: string): Promise<Utxo[]> {
      ...
      try {
          ...
          const url = `${this.providerURL}/LTC/mainnet/address/${address}/?unspent=true`
  ...
  async buildSignedPayment(paymentOption: PaymentOption): Promise<Transaction> {
    ...
    if (totalInput < totalOutput) {
      ...
      throw new Error(`Insufficient ${'LTC'} funds to pay invoice`)
    }
