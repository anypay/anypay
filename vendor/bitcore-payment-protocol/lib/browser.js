...
PaymentProtocol.verifyCertChain = function(chain, sigHashAlg) {
  ...
  return chain.every(function(cert, i) {
    ...
    if (sigHashAlg !== 'none') {
      var sigAlgId = c.signatureAlgorithm.algorithm.join('.')
      var sigAlg = PaymentProtocol.getAlgorithm(sigAlgId).split('_')
      // If the signature algorithm is plain RSA, the cert is invalid
      if (sigAlg.length == 1 && sigAlg[0] == 'RSA') {
        sigVerified = false
      } else {
        var jsrsaSig = new KJUR.crypto.Signature({
          alg: sigAlg[1].toLowerCase() + 'with' + sigAlg[0],
          prov: 'cryptojs/jsrsa'
        });
