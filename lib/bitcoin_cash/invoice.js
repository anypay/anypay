  ...
  return new Promise((resolve, reject) => {
    ...
    return Account.findOne({ where: { id: accountId }}).then(account => {
      ...
      return BitcoinCashAddressService
        .getNewAddress(dollarAmount, account.bitcoin_cash_address).then(address => {

        let bitcoinCashAmount = BitcoinCashPrice.convertDollarsToDash(dollarAmount).toFixed(5);
