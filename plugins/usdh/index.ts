...
export async function getNewAddress(r: any) {
  ...
  let record = await models.Hdkeyaddresses.create({
    ...
    xpub_key:process.env.USDH_HD_PUBLIC_KEY

  })

  var bchAddress = deriveAddress(process.env.USDH_HD_PUBLIC_KEY, record.id);
...
async function createInvoice(accountId: number, amount: number) {
  ...
  let invoice = await generateInvoice(accountId, amount, 'USDH');
