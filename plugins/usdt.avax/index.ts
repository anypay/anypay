...
export async function validateAddress(address: string): Promise<boolean> {
  ...
  return avalanche.isAddress({ address })
...
class BitcoreTransaction {
  ...
  constructor(hex) {
      ...
      if (hex.length === 66) {
        ...
        //this.transaction = avalanche.decodeTransactionHex({ transactionHex: hex })
...
export async function verifyPayment({ payment_option, transaction: {tx: hex}, protocol }: VerifyPayment) {
  ...
  */
  ...
  try {
    ...
    /*
    const output: any = avalanche.parseUSDCOutput({ transactionHex: hex })
    ...
    */
    ...
    const { parsed, full } = await avalanche.fetchERC20Transfer({ txid: hex })

    log.info('usdc.avalanche.fetchERC20Transfer.result', { parsed, full })
...
export async function broadcastTx(txhex: string) {
  ...
  if (txhex.length === 66) {
    ...
    console.log('skip USDC avalanche broadcast of txid', { txid: txhex })
  ...
  console.log('avalanche.provider.sendTransaction.result', transmitResult)
