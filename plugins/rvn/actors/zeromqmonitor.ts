...
if (!process.env.RAVENCOIN_ZEROMQ_URL) {

  console.log('RAVENCOIN_ZEROMQ_URL environment variable required');
...
sock.connect(process.env.RAVENCOIN_ZEROMQ_URL);
...
/*
 * rvn.rawtx event indicates to the system that the bitcoincash full
 * node has received a valid transaction on the p2p network. This
 * actor publishes the raw transaction data buffer to AMQP.
 *
 * rvn.rawtx may be configured by providing the AMQP_ROUTING_KEY and
 * AMQP_EXCHANGE environment variables at runtime.
 *
 */

const exchange = process.env.AMQP_EXCHANGE || 'anypay.rvn';
...
async function start() {
  ...
  events.forEach(async event => {
    ...
    await channel.assertQueue(`anypay.rvn.${event}`);

    await channel.bindQueue(`anypay.rvn.${event}`, exchange, `rvn.${event}`);
