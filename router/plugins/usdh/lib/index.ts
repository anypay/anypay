...
export async function getAddressRouteFromTx(tx):Promise<Route>{
  ...
  for (let i=0; i < outputs.length; i++) {
    ...
    try {
      ...
      route = await lookupOutputFromInput('USDH', address)
...
function bchToSatoshis(bchAmount): number{
  return bchAmount*100000000 | 0;
}

export function derivePrivateKey(pk, nonce): string{
...
export async function sendSLPToken(tokenId, route, pk){
  ...
  console.log("Send SLP Token txid: ", tokenReceiverAddress);
