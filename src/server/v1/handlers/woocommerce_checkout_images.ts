...
export async function show(request: AuthenticatedRequest | Request, h: ResponseToolkit) {
  ...
  try {
    ...
    let image = await getAccountSetting((request as AuthenticatedRequest).account.id, 'woocommerce.checkout.image', { default: 'ANYPAY' })
...
export async function update(request: AuthenticatedRequest | Request, h: ResponseToolkit) {
  ...
  if (url) {
    ...
    await setAccountSetting((request as AuthenticatedRequest).account.id, 'woocommerce.checkout.image', name)
